...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const documents = await sanityFetch<SitemapDocument[]>(
    `*[_type in ["page", "program", "caseStudy"] && defined(slug.current)]{
      _type,
      "slug": slug.current,
  ...
  const contentRoutes: MetadataRoute.Sitemap = documents.flatMap((document) => {
    ...
    const prefix =
      document._type === 'caseStudy' ? '/work/' : document._type === 'program' ? '/strengths/' : '/'
    ...
    return [
      {
        ...
        priority: document._type === 'caseStudy' ? 0.8 : 0.7,
      },
    ]
  })
