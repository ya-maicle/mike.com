...
check_response_time() {
  ...
  local response_time=$(curl -s -w "%{time_total}" "$url/api/health" -o /dev/null)
  local response_time_ms=$(echo "$response_time * 1000" | bc 2>/dev/null || echo "unknown")
  
  if [ "$response_time_ms" != "unknown" ]; then
    echo "✅ $name response time: ${response_time_ms}ms"
    
    # Warn if response time is over 2 seconds
    if (( $(echo "$response_time > 2.0" | bc -l 2>/dev/null || echo "0") )); then
      echo "   ⚠️  Response time is slow (>2s)"
    fi
  else
    echo "⚠️  Could not measure $name response time"
  fi
  echo ""
}
...
check_response_time "$STAGING_URL" "Staging"
...
check_response_time "$PRODUCTION_URL" "Production"
